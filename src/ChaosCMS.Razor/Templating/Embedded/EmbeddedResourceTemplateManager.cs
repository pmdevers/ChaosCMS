...
namespace ChaosCMS.Razor.Templating.Embedded
{
	public class EmbeddedResourceTemplateManager : ITemplateManager
	{
		...
		public ITemplateSource Resolve(string key)
		{
			...
			using (var stream = assembly.GetManifestResourceStream(this.RootType.Namespace + "." + key + ".cshtml"))
			{
				if (stream == null)
				{
					throw new ChaosRazorException(string.Format("Couldn't load resource '{0}.{1}.cshtml' from assembly {2}", this.RootType.Namespace, key, this.RootType.AssemblyQualifiedName));
				}
